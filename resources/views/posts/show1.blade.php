            ...
            <div class="px-8 py-6 bg-gradient-to-r from-gray-50 to-blue-50 border-t border-gray-200">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <div class="flex items-center space-x-3">
                        <!-- Like Button -->
                        @auth
                            <form action="{{ route('posts.like.toggle', $post) }}" method="POST" class="inline-block">
                                @csrf
                                <button type="submit" 
                                        class="group flex items-center space-x-2 px-5 py-3 rounded-xl transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5
                                        {{ $post->isLikedBy(auth()->user()) 
                                            ? 'bg-red-500 text-white border-2 border-red-500' 
                                            : 'bg-white border-2 border-gray-200 hover:border-red-300 hover:bg-red-50' }}">
                                    <svg class="w-6 h-6 transition {{ $post->isLikedBy(auth()->user()) ? 'text-white' : 'text-gray-600 group-hover:text-red-500' }}" 
                                         fill="{{ $post->isLikedBy(auth()->user()) ? 'currentColor' : 'none' }}" 
                                         stroke="currentColor" 
                                         viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path>
                                    </svg>
                                    <span class="text-sm font-bold {{ $post->isLikedBy(auth()->user()) ? 'text-white' : 'text-gray-700 group-hover:text-red-600' }}">
                                        {{ $post->likes_count }}
                                    </span>
                                </button>
                            </form>
                            ...
                            <!-- Bookmark Button -->
                            <form action="{{ route('posts.bookmark.toggle', $post) }}" method="POST" class="inline-block">
                                @csrf
                                <button type="submit" 
                                        class="group flex items-center space-x-2 px-5 py-3 rounded-xl transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5
                                    ...
                                    <svg class="w-6 h-6 transition {{ $post->isBookmarkedBy(auth()->user()) ? 'text-white' : 'text-gray-600 group-hover:text-amber-600' }}" 
                                         fill="{{ $post->isBookmarkedBy(auth()->user()) ? 'currentColor' : 'none' }}" 
                                         stroke="currentColor" 
                                         viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path>
                                    </svg>
                                    <span class="text-sm font-bold {{ $post->isBookmarkedBy(auth()->user()) ? 'text-white' : 'text-gray-700 group-hover:text-amber-600' }}">
                                        {{ $post->isBookmarkedBy(auth()->user()) ? 'Guardado' : 'Guardar' }}
                                    </span>
                                </button>
                            </form>
                        @else
                            <!-- Guest users -->
                            <a href="{{ route('login') }}" 
                               class="group flex items-center space-x-2 px-5 py-3 bg-white border-2 border-gray-200 rounded-xl hover:border-red-300 hover:bg-red-50 transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                            ...
                            <a href="{{ route('login') }}" 
                               class="group flex items-center space-x-2 px-5 py-3 bg-white border-2 border-gray-200 rounded-xl hover:border-amber-300 hover:bg-amber-50 transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                    ...
                    <button onclick="sharePost()" 
                            class="flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-techgap-600 to-techgap-700 text-white rounded-xl hover:from-techgap-700 hover:to-techgap-800 transition-all shadow-lg hover:shadow-xl font-semibold transform hover:-translate-y-0.5">
